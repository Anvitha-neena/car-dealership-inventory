...
const THEME_KEY = 'motorstock-theme';
...
function storedTheme(): 'dark' | 'light' { return localStorage.getItem(THEME_KEY) === 'light' ? 'light' : 'dark'; }
...
export default function App() {
  ...
  const [trashVehicles, setTrashVehicles] = useState<Vehicle[]>([]);
  ...
  const loadTrash = async (activeSession = session) => {
    if (!activeSession || activeSession.user.role !== 'admin') return;
    setLoading(true); setError('');
    try { setTrashVehicles(await api.trash(activeSession.token)); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to load archived vehicles.'); }
    finally { setLoading(false); }
  };
  ...
  const remove = async () => { if (!session || !deleteCandidate) return; const id = deleteCandidate.id; setPendingAction(`archive:${id}`); setError(''); try { await api.deleteVehicle(session.token, id); setVehicles((items) => items.filter((item) => item.id !== id)); setTrashVehicles((items) => [{ ...deleteCandidate, deletedAt: new Date().toISOString() }, ...items]); setDeleteCandidate(null); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to move vehicle to trash.'); } finally { setPendingAction(null); } };
  const restore = async (id: string) => { if (!session) return; setPendingAction(`restore:${id}`); setError(''); try { const vehicle = await api.restoreVehicle(session.token, id); setTrashVehicles((items) => items.filter((item) => item.id !== id)); setVehicles((items) => [vehicle, ...items]); } catch (caught) { setError(caught instanceof Error ? caught.message : 'Unable to restore vehicle.'); } finally { setPendingAction(null); } };
  ...
  return <main className={`app-shell theme-${theme}`}><header><div><p className="eyebrow">MOTORSTOCK</p><h1>{isAdmin ? 'Inventory control' : 'Available inventory'}</h1></div><div className="profile"><span>{session.user.name} · {session.user.role}</span><button className="theme-toggle" onClick={toggleTheme} aria-label="Toggle colour theme">{theme === 'dark' ? '☀ Light' : '☾ Dark'}</button>{isAdmin && <button className="trash-toggle" onClick={() => { const next = !showTrash; setShowTrash(next); if (next) void loadTrash(); }}>{showTrash ? 'Back to inventory' : `Trash (${trashVehicles.length})`}</button>}<button onClick={() => { localStorage.removeItem(SESSION_KEY); setSession(null); }}>Sign out</button></div></header><section className="dashboard-grid"><article><p className="eyebrow">VEHICLES</p><strong>{vehicles.length}</strong><span>{isAdmin ? 'tracked models' : 'available choices'}</span></article><article><p className="eyebrow">UNITS IN STOCK</p><strong>{totalUnits}</strong><span>ready for purchase</span></article>{isAdmin ? <><article><p className="eyebrow">OUT OF STOCK</p><strong>{zeroStock}</strong><span>need restocking</span></article><article><p className="eyebrow">INVENTORY VALUE</p><strong>${inventoryValue.toLocaleString()}</strong><span>current stock value</span></article></> : <article><p className="eyebrow">WELCOME</p><strong>Ready</strong><span>find your next vehicle</span></article>}</section>
    ...
    {error && <p className="error" role="alert">{error}</p>}{showTrash ? <section className="trash-view"><div><p className="eyebrow">ADMIN TRASH</p><h2>Archived vehicles</h2><p className="muted">Restore a vehicle to return it to active inventory.</p></div><section className="vehicle-grid">{trashVehicles.map((vehicle) => <TrashCard key={vehicle.id} vehicle={vehicle} onRestore={(id) => void restore(id)} pending={pendingAction === `restore:${vehicle.id}`} />)}</section>{!loading && !trashVehicles.length && <p className="muted">Trash is empty.</p>}</section> : <>{loading && !vehicles.length ? <p className="muted">Loading inventory…</p> : <section className="vehicle-grid">{vehicles.map((vehicle) => <VehicleCard key={vehicle.id} vehicle={vehicle} onPurchase={(id) => void purchase(id)} isAdmin={isAdmin} onEdit={edit} onDelete={(id) => setDeleteCandidate(vehicles.find((item) => item.id === id) ?? null)} onRestock={(id, quantity) => void restock(id, quantity)} pendingAction={pendingAction?.endsWith(`:${vehicle.id}`) ? pendingAction.split(':')[0] as 'purchase' | 'restock' | 'archive' : null} />)}</section>}{!loading && !vehicles.length && <p className="muted">No vehicles match your filters.</p>}</>}
    {deleteCandidate && <div className="dialog-backdrop" role="presentation"><section className="dialog" role="dialog" aria-modal="true" aria-labelledby="delete-title"><p className="eyebrow">MOVE TO TRASH</p><h2 id="delete-title">Move {deleteCandidate.make} {deleteCandidate.model} to trash?</h2><p className="muted">The vehicle will be hidden from inventory and can be restored later.</p><div className="dialog-actions"><button onClick={() => setDeleteCandidate(null)} disabled={Boolean(pendingAction)}>Cancel</button><button className="danger" onClick={() => void remove()} disabled={Boolean(pendingAction)}>{pendingAction ? 'Moving…' : 'Move to trash'}</button></div></section></div>}
